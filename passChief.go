...
var path = "/home/axel/password-Chief/"
...
func main(){
	...
	for vpass > 0 {
		...
		if verify_pass(user, pass) == false {
	...
	for dChoice > 0 {	
		...
		if response == "1" {
			...
			add_login(user, pass, lUser, lPass)
		}else if response  == "2" { 
				printContent(user, pass)
		}else if response == "3"{
			fmt.Print("Good-Bye ! \n")
			dChoice = -1
		}else{
...
func verify_pass(user string, pass string) bool{
...
func printContent(user string, pass string) int{
	h := md5.New()
	io.WriteString(h, pass)
	Hpass := h.Sum(nil)
...
func valideacc(user string) bool{
...
func add_user(user string, pass string) int{
...
func add_login(user string, pass string, login string, lpass string) {
